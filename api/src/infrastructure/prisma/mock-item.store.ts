...
export class MockItemStore {
  ...
  findUnique = async ({ where }: { where: Prisma.ItemWhereUniqueInput }) => {
    if (where.id !== undefined) return this.items.find((i) => i.id === where.id) ?? null;
    return null;
  };
  findMany = async ({ where }: { where?: Prisma.ItemWhereInput } = {}) => {
    if (!where) return [...this.items];
    return this.items.filter((i) => {
      if (where.pantryId !== undefined && i.pantryId !== where.pantryId) return false;
  ...
  create = async ({
    ...
    if (typeof pantryId !== "number") {
      throw new Error("Invalid ItemCreateInput: pantryId or pantry.connect.id required");
    }
    const name = "name" in data ? data.name : "";
    const quantity = "quantity" in data && typeof data.quantity === "number" ? data.quantity : 1;
    const unit = "unit" in data ? data.unit : "";
    const category = "category" in data ? data.category : "Food";
    const expiresAt = "expiresAt" in data ? (data.expiresAt as Date | null | undefined) : null;
    const item: Item = {
      id: this.idSeq++,
      pantryId,
      name,
  ...
  update = async ({
    ...
    const item = findById(this.items, itemId);
