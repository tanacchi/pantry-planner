...
export class MockPantryStore {
  ...
  findUnique = async ({ where }: { where: Prisma.PantryWhereUniqueInput }) => {
    if (where.id !== undefined) return this.pantries.find((p) => p.id === where.id) ?? null;
    return null;
  };
  ...
  create = async ({
    ...
    if ("userId" in data) {
      ...
      throw new Error("Invalid PantryCreateInput: userId or user.connect.id required");
    }
    const pantry: Pantry = {
      id: this.idSeq++,
      userId,
  ...
  update = async ({
    ...
    const pantryId = where.id;
    if (pantryId === undefined) throw new Error("Pantry id is required");
    const pantry = findById(this.pantries, pantryId);
