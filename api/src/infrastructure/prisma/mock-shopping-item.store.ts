...
export class MockShoppingItemStore {
  ...
  update = async ({
    ...
    const shoppingItemId = where.id;
    if (shoppingItemId === undefined) throw new Error("Shopping item id is required");
    const shoppingItem = findById(this.shoppingItems, shoppingItemId);
